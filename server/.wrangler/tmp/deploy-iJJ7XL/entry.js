...
${cause.stack}`;
    ...
    __name(toOpenApiError, "toOpenApiError");
    __name(toTransportFailure, "toTransportFailure");
...
var DEFAULT_VERSION, VSN_1_0_0, VSN_2_0_0, DEFAULT_VSN, DEFAULT_TIMEOUT, DEFAULT_POSTGRES_CHANGES_WAIT_TIMEOUT, POSTGRES_CHANGES_WAIT_ERROR_GRACE, MAX_PUSH_BUFFER_SIZE, CHANNEL_STATES, CHANNEL_EVENTS, CONNECTION_STATE;
var init_constants = __esm({
  "node_modules/@supabase/realtime-js/dist/module/lib/constants.js"() {
    ...
    DEFAULT_TIMEOUT = 1e4;
    DEFAULT_POSTGRES_CHANGES_WAIT_TIMEOUT = 15e3;
    POSTGRES_CHANGES_WAIT_ERROR_GRACE = 1e4;
...
var PostgrestReservedCharsRegexp2, needsQuoting, quote, serializeScalar, serializeIsValue, serialize, RealtimePostgresFilterBuilder, postgresChangesFilter;
var init_RealtimePostgresFilterBuilder = __esm({
  "node_modules/@supabase/realtime-js/dist/module/RealtimePostgresFilterBuilder.js"() {
    ...
    postgresChangesFilter = /* @__PURE__ */ __name(() => new RealtimePostgresFilterBuilder(), "postgresChangesFilter");
  }
});
...
var ApiError = class _ApiError extends Error {
  ...
  static badRequest(message, data) {
    return new _ApiError(400, message, data);
  }
...
var PROFILE_FIELDS = ["name", "username", "bio", "country", "avatar"];
...
async function upsertProfileFromAuthUser(supabase, authUser) {
...
__name(upsertProfileFromAuthUser, "upsertProfileFromAuthUser");
...
async function updateProfileByUserId(supabase, userId, fields) {
...
__name(updateProfileByUserId, "updateProfileByUserId");
async function getFavoritesByUserId(supabase, userId) {
  const { data, error } = await supabase.from("favorites").select("*").eq("user_id", userId).order("created_at", { ascending: false });
  if (error) throw error;
  return data || [];
}
__name(getFavoritesByUserId, "getFavoritesByUserId");
async function addFavoriteByUserId(supabase, userId, favorite) {
  const { data, error } = await supabase.from("favorites").upsert({ user_id: userId, ...favorite }, { onConflict: "user_id,game_id" }).select().single();
  if (error) throw error;
  return data;
}
__name(addFavoriteByUserId, "addFavoriteByUserId");
async function removeFavoriteByGameId(supabase, userId, gameId) {
  const { error } = await supabase.from("favorites").delete().eq("user_id", userId).eq("game_id", gameId);
  if (error) throw error;
}
__name(removeFavoriteByGameId, "removeFavoriteByGameId");
async function getDownloadHistoryByUserId(supabase, userId) {
  const { data, error } = await supabase.from("download_history").select("*").eq("user_id", userId).order("downloaded_at", { ascending: false });
  if (error) throw error;
  return data || [];
}
__name(getDownloadHistoryByUserId, "getDownloadHistoryByUserId");
async function addDownloadRecordByUserId(supabase, userId, record, ipAddress) {
  const { data, error } = await supabase.from("download_history").upsert({ user_id: userId, ...record, downloaded_at: (/* @__PURE__ */ new Date()).toISOString(), ip_address: ipAddress }, { onConflict: "user_id,game_id" }).select().single();
  if (error) throw error;
  return data;
}
__name(addDownloadRecordByUserId, "addDownloadRecordByUserId");
async function removeDownloadRecordById(supabase, userId, recordId) {
  const { error } = await supabase.from("download_history").delete().eq("user_id", userId).eq("id", recordId);
  if (error) throw error;
}
__name(removeDownloadRecordById, "removeDownloadRecordById");
async function clearDownloadHistoryByUserId(supabase, userId) {
  const { error } = await supabase.from("download_history").delete().eq("user_id", userId);
  if (error) throw error;
}
__name(clearDownloadHistoryByUserId, "clearDownloadHistoryByUserId");

// src/services/authService.ts
async function loginWithAccessToken(supabase, token) {
  if (!token) throw new Error("Access token is required");
  const { data, error } = await supabase.auth.getUser(token);
  if (error || !data.user) throw new Error("Invalid or expired session");
  return upsertProfileFromAuthUser(supabase, data.user);
}
__name(loginWithAccessToken, "loginWithAccessToken");
...
function getSupabaseFromEnv(c) {
  return c.get("supabase");
}
__name(getSupabaseFromEnv, "getSupabaseFromEnv");
...
function blockBanned(c, next) {
  const user = c.get("user");
  if (user?.is_banned) throw new ApiError(403, "Your account has been suspended");
  return next();
}
__name(blockBanned, "blockBanned");

// src/routes/auth.ts
var authRoutes = createRouter().post("/google", login).get("/user", authenticate, getMe).post("/logout", authenticate, logout);

// src/services/gamesService.ts
async function listGames(supabase, query) {
  const {
    page: pageParam = 1,
    limit: limitParam = 12,
    sort = "latest",
...
async function populateGameDetails(supabase, game) {
  if (!game) return null;
  const [screenshotsRes, linksRes] = await Promise.all([
    supabase.from("screenshots").select("image_url, position").eq("game_id", game.id).order("position", { ascending: true }),
    supabase.from("download_links").select("id, provider, mirror_name, file_size, password, is_active, sort_order").eq("game_id", game.id).eq("is_active", true).order("sort_order", { ascending: true })
  ]);
  game.screenshots = (screenshotsRes.data || []).map((shot) => ({
    url: shot.image_url
  }));
  const mirrors = (linksRes.data || []).map((link) => ({
    id: link.id,
...
__name(populateGameDetails, "populateGameDetails");
async function getGameBySlug(supabase, slug, options = {}) {
...
__name(getGameBySlug, "getGameBySlug");
async function getGameById(supabase, id, options = {}) {
...
async function getRelatedGames(supabase, gameId, genreId, limit = 4) {
  if (!genreId) return [];
  const { data, error } = await supabase.from("games").select("*").eq("genre_id", genreId).neq("id", gameId).limit(limit);
  if (error) throw error;
  return data || [];
}
__name(getRelatedGames, "getRelatedGames");
async function getGamesByCollection(supabase, collectionId) {
  const { data, error } = await supabase.from("collection_games").select("games(*)").eq("collection_id", collectionId);
  if (error) throw error;
  return data?.map((c) => c.games).filter(Boolean) || [];
}
__name(getGamesByCollection, "getGamesByCollection");

// src/routes/games.ts
var gamesRoutes = createRouter();
gamesRoutes.get("/trending", async (c) => {
  const supabase = c.get("supabase");
  const limit = c.req.query("limit") ? Number(c.req.query("limit")) : void 0;
...
// src/services/fixesService.ts
async function listFixes(supabase, query) {
...
__name(listFixes, "listFixes");
async function getFixBySlug(supabase, slug, options = {}) {
...
__name(getFixBySlug, "getFixBySlug");
async function getRelatedFixes(supabase, fix) {
  if (!fix.category) return [];
  const { data, error } = await supabase.from("fix_articles").select("*").eq("category", fix.category).neq("id", fix.id).limit(4);
  if (error) {
...
__name(getRelatedFixes, "getRelatedFixes");
async function getFixCategories(supabase) {
...
__name(getFixCategories, "getFixCategories");

// src/routes/fixes.ts
var fixesRoutes = createRouter();
...
async function listGuides(supabase, query) {
  const { page: pageParam = 1, limit: limitParam = 12, sort = "latest", search, category } = query;
...
__name(listGuides, "listGuides");
async function getGuideBySlug(supabase, slug, options = {}) {
...
__name(getGuideBySlug, "getGuideBySlug");
async function getRelatedGuides(supabase, guide) {
  if (!guide.category) return [];
  const { data, error } = await supabase.from("guides").select("*").eq("category", guide.category).neq("id", guide.id).limit(4);
  if (error) throw error;
  return data || [];
}
__name(getRelatedGuides, "getRelatedGuides");
async function getGuideCategories(supabase) {
  const { data, error } = await supabase.from("guides").select("category").not("category", "is", null);
  if (error) throw error;
  const categories = [...new Set(data.map((d) => d.category).filter(Boolean))];
  return categories;
}
__name(getGuideCategories, "getGuideCategories");

// src/routes/guides.ts
var guidesRoutes = createRouter();
...
var categoriesRoutes = createRouter();
...
async function listCollections(supabase) {
...
async function getCollectionBySlug(supabase, slug) {
  const { data, error } = await supabase.from("collections").select("*").eq("slug", slug).maybeSingle();
  if (error) throw error;
  return data;
}
__name(getCollectionBySlug, "getCollectionBySlug");

// src/routes/collections.ts
var collectionsRoutes = createRouter();
...
async function searchGames(supabase, query) {
...
__name(getFavorites, "getFavorites");
async function addFavorite(c) {
  try {
    const supabase = c.get("supabase");
    const body = await c.req.json();
...
async function getDownloadHistory(c) {
  try {
    const supabase = c.get("supabase");
...
__name(getDownloadHistory, "getDownloadHistory");
async function addDownloadRecord(c) {
  try {
    const supabase = c.get("supabase");
    const body = await c.req.json();
    if (!validateUuid(body.game_id)) {
...
__name(addDownloadRecord, "addDownloadRecord");
async function removeDownloadRecord(c) {
  try {
    const supabase = c.get("supabase");
...
__name(removeDownloadRecord, "removeDownloadRecord");
async function clearDownloadHistory(c) {
  try {
    const supabase = c.get("supabase");
...
__name(clearDownloadHistory, "clearDownloadHistory");

// src/routes/users.ts
var usersRoutes = createRouter();
...
// src/services/steamFreeService.ts
async function getSteamFreeContent(supabase) {
  const { data, error } = await supabase.from("steam_free_games").select("*").order("created_at", { ascending: false });
  if (error) throw error;
  return data || [];
}
__name(getSteamFreeContent, "getSteamFreeContent");

// src/routes/steamFree.ts
var steamFreeRoutes = createRouter();
...
async function getCommentsByGame(supabase, gameId) {
  try {
    ...
    const { data, error } = await supabase.from("comments").select("*, user:users(id, name, username, avatar)").eq("game_id", targetGameId).eq("status", "approved").order("created_at", { ascending: false });
    if (error) {
    ...
    console.error("getCommentsByGame error:", { gameId, error: err instanceof Error ? err.message : String(err), stack: err instanceof Error ? err.stack : void 0 });
    return [];
  }
}
__name(getCommentsByGame, "getCommentsByGame");
async function addComment(supabase, gameId, userId, content, parentId = null) {
  try {
    ...
    if (!game) throw new ApiError(404, "Game not found");
    if (parentId) {
      const { data: parent, error: parentError } = await supabase.from("comments").select("id").eq("id", parentId).maybeSingle();
    ...
    }).select("*, user:users(id, name, username, avatar)").single();
    if (error) throw error;
...
async function updateComment(supabase, commentId, userId, content) {
  try {
    ...
    const { data: existing, error: findError } = await supabase.from("comments").select("*").eq("id", commentId).maybeSingle();
    if (findError) throw findError;
    if (!existing) throw new ApiError(404, "Comment not found");
    if (existing.user_id !== userId) {
      throw new ApiError(403, "You can only edit your own comments");
    }
    const { data, error } = await supabase.from("comments").update({ content: content.trim(), updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", commentId).select("*, user:users(id, name, username, avatar)").single();
    if (error) throw error;
    return toComment(data);
  } catch (err) {
...
async function deleteComment(supabase, commentId, userId) {
  try {
    const { data: existing, error: findError } = await supabase.from("comments").select("id, user_id").eq("id", commentId).maybeSingle();
    if (findError) throw findError;
    if (!existing) throw new ApiError(404, "Comment not found");
    if (existing.user_id !== userId) {
...
var commentsRoutes = createRouter();
...
async function getRatingSummary(supabase, gameId) {
  try {
    let targetGameId = gameId;
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(gameId);
    if (!isUuid) {
    ...
    const { data, error } = await supabase.from("ratings").select("user_id, rating").eq("game_id", targetGameId);
    if (error) {
    ...
    console.error("getRatingSummary error:", { gameId, error: err instanceof Error ? err.message : String(err), stack: err instanceof Error ? err.stack : void 0 });
    return { average_rating: 0, rating_count: 0, user_rating: null };
  }
}
__name(getRatingSummary, "getRatingSummary");
async function rateGame(supabase, gameId, userId, rating) {
...
__name(rateGame, "rateGame");

// src/routes/ratings.ts
var ratingsRoutes = createRouter();
...
ratingsRoutes.post("/", authenticate, blockBanned, async (c) => {
  ...
  if (!gameId || rating === void 0) throw new ApiError(400, "game_id and rating are required");
...
__name(toMirror, "toMirror");
async function getMirrorsByGame(supabase, gameId) {
  let targetGameId = gameId;
  const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(gameId);
  if (!isUuid) {
  ...
  const { data, error } = await supabase.from("download_links").select("*").eq("game_id", targetGameId).eq("is_active", true).order("sort_order", { ascending: true });
  if (error) {
...
__name(getMirrorsByGame, "getMirrorsByGame");
async function getMirrorById(supabase, id) {
  const { data, error } = await supabase.from("download_links").select("*").eq("id", id).maybeSingle();
  if (error) throw error;
  return data || null;
}
__name(getMirrorById, "getMirrorById");
async function startDownload(supabase, gameId, mirrorId) {
  let targetGameId = gameId;
  const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(gameId);
  if (!isUuid) {
    const { data: g } = await supabase.from("games").select("id").eq("slug", gameId).maybeSingle();
  ...
  const { data: game, error: gameError } = await supabase.from("games").select("id, title, slug, cover_image, downloads").eq("id", targetGameId).maybeSingle();
  if (gameError) throw gameError;
  if (!game) throw new ApiError(404, "Game not found");
...
__name(startDownload, "startDownload");
async function redirectToMirror(supabase, mirrorId) {
  const mirror = await getMirrorById(supabase, mirrorId);
  if (!mirror || !mirror.is_active) {
    throw new ApiError(404, "Download link not found or inactive");
  }
  ...
  const { data: game } = await supabase.from("games").select("downloads").eq("id", mirror.game_id).maybeSingle();
  if (game) {
    await supabase.from("games").update({ downloads: (game.downloads || 0) + 1 }).eq("id", mirror.game_id);
  }
  return { url: mirror.download_url, mirror };
}
__name(redirectToMirror, "redirectToMirror");

// src/routes/downloads.ts
var downloadsRoutes = createRouter();
...
app.use("*", cors({
  origin: /* @__PURE__ */ __name((origin, c) => {
    const allowed = (c.env.CLIENT_ORIGINS || "").split(",").map((o) => o.trim()).filter(Boolean);
...
app.get("/health", (c) => c.json({ ok: true, timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
app.use("*", async (c, next) => {
  if (c.req.path === "/health") return next();
  const { createClient: createClient2 } = await Promise.resolve().then(() => (init_dist4(), dist_exports));
  const supabase = createClient2(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY, {
...
app.onError((err, c) => {
  ...
  if (err instanceof ApiError) return c.json({ success: false, message: err.message, ...err.data ? { data: err.data } : {} }, err.statusCode);
