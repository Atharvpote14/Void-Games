import { Router } from 'express'
import { authenticate, requireAdmin } from '../middleware/auth.js'
import { getDashboard } from '../controllers/adminController.js'
import {
  getGames,
  getGame,
  createGame,
  updateGame,
  deleteGame,
} from '../controllers/adminGamesController.js'
import {
  getCategories,
  getCategory,
  createCategory,
  updateCategory,
  deleteCategory,
} from '../controllers/adminCategoriesController.js'
import {
  getCollections,
  getCollection,
  createCollection,
  updateCollection,
  deleteCollection,
  getGamePicker,
} from '../controllers/adminCollectionsController.js'
import {
  getGuides,
  getGuide,
  createGuide,
  updateGuide,
  deleteGuide,
  getFixes,
  getFix,
  createFix,
  updateFix,
  deleteFix,
} from '../controllers/adminArticlesController.js'
import {
  getUsers,
  getUser,
  updateUser,
  deleteUser,
} from '../controllers/adminUsersController.js'
import {
  getReports,
  getReport,
  updateReportStatus,
  deleteReport,
} from '../controllers/adminReportsController.js'
import {
  getUnbanRequests,
  getUnbanRequest,
  reviewUnbanRequest,
  deleteUnbanRequest,
} from '../controllers/adminUnbanRequestsController.js'
import {
  getSuggestions,
  getSuggestion,
  updateSuggestion,
  deleteSuggestion,
} from '../controllers/adminSuggestionsController.js'
import { getAnalyticsData } from '../controllers/adminAnalyticsController.js'
import {
  getSteamFreeContent,
  updateSteamFreeVideo,
  createStep,
  updateStep,
  deleteStep,
} from '../controllers/adminSteamFreeController.js'

const router = Router()

router.use(authenticate, requireAdmin)

router.get('/dashboard', getDashboard)
router.get('/analytics', getAnalyticsData)

router.get('/games/picker', getGamePicker)
router.get('/games', getGames)
router.get('/games/:id', getGame)
router.get('/game/:id', getGame)
router.post('/games', createGame)
router.post('/game', createGame)
router.patch('/games/:id', updateGame)
router.patch('/game/:id', updateGame)
router.put('/games/:id', updateGame)
router.put('/game/:id', updateGame)
router.delete('/games/:id', deleteGame)
router.delete('/game/:id', deleteGame)

router.get('/categories', getCategories)
router.get('/category', getCategories)
router.get('/categories/:id', getCategory)
router.get('/category/:id', getCategory)
router.post('/category', createCategory)
router.post('/categories', createCategory)
router.patch('/category/:id', updateCategory)
router.patch('/categories/:id', updateCategory)
router.put('/category/:id', updateCategory)
router.put('/categories/:id', updateCategory)
router.delete('/category/:id', deleteCategory)
router.delete('/categories/:id', deleteCategory)

router.get('/collections', getCollections)
router.get('/collection', getCollections)
router.get('/collections/:id', getCollection)
router.get('/collection/:id', getCollection)
router.post('/collection', createCollection)
router.post('/collections', createCollection)
router.patch('/collection/:id', updateCollection)
router.patch('/collections/:id', updateCollection)
router.put('/collection/:id', updateCollection)
router.put('/collections/:id', updateCollection)
router.delete('/collection/:id', deleteCollection)
router.delete('/collections/:id', deleteCollection)

router.get('/guides', getGuides)
router.get('/guides/:id', getGuide)
router.get('/guide/:id', getGuide)
router.post('/guide', createGuide)
router.post('/guides', createGuide)
router.patch('/guide/:id', updateGuide)
router.patch('/guides/:id', updateGuide)
router.put('/guide/:id', updateGuide)
router.put('/guides/:id', updateGuide)
router.delete('/guide/:id', deleteGuide)
router.delete('/guides/:id', deleteGuide)

router.get('/fixes', getFixes)
router.get('/fixes/:id', getFix)
router.get('/fix/:id', getFix)
router.post('/fix', createFix)
router.post('/fixes', createFix)
router.patch('/fix/:id', updateFix)
router.patch('/fixes/:id', updateFix)
router.put('/fix/:id', updateFix)
router.put('/fixes/:id', updateFix)
router.delete('/fix/:id', deleteFix)
router.delete('/fixes/:id', deleteFix)

router.get('/users', getUsers)
router.get('/users/:id', getUser)
router.get('/user/:id', getUser)
router.patch('/user/:id', updateUser)
router.patch('/users/:id', updateUser)
router.put('/user/:id', updateUser)
router.put('/users/:id', updateUser)
router.delete('/user/:id', deleteUser)
router.delete('/users/:id', deleteUser)

router.get('/reports', getReports)
router.get('/reports/:id', getReport)
router.get('/report/:id', getReport)
router.patch('/report/:id', updateReportStatus)
router.patch('/reports/:id', updateReportStatus)
router.put('/report/:id', updateReportStatus)
router.put('/reports/:id', updateReportStatus)
router.delete('/report/:id', deleteReport)
router.delete('/reports/:id', deleteReport)

router.get('/unban-requests', getUnbanRequests)
router.get('/unban-requests/:id', getUnbanRequest)
router.get('/unban-request/:id', getUnbanRequest)
router.patch('/unban-request/:id', reviewUnbanRequest)
router.patch('/unban-requests/:id', reviewUnbanRequest)
router.put('/unban-request/:id', reviewUnbanRequest)
router.put('/unban-requests/:id', reviewUnbanRequest)
router.delete('/unban-request/:id', deleteUnbanRequest)
router.delete('/unban-requests/:id', deleteUnbanRequest)

router.get('/suggestions', getSuggestions)
router.get('/suggestions/:id', getSuggestion)
router.get('/suggestion/:id', getSuggestion)
router.patch('/suggestion/:id', updateSuggestion)
router.patch('/suggestions/:id', updateSuggestion)
router.put('/suggestion/:id', updateSuggestion)
router.put('/suggestions/:id', updateSuggestion)
router.delete('/suggestion/:id', deleteSuggestion)
router.delete('/suggestions/:id', deleteSuggestion)

router.get('/steam-free', getSteamFreeContent)
router.patch('/steam-free/video', updateSteamFreeVideo)
router.put('/steam-free/video', updateSteamFreeVideo)
router.post('/steam-free/step', createStep)
router.post('/steam-free/steps', createStep)
router.patch('/steam-free/step/:id', updateStep)
router.patch('/steam-free/steps/:id', updateStep)
router.put('/steam-free/step/:id', updateStep)
router.put('/steam-free/steps/:id', updateStep)
router.delete('/steam-free/step/:id', deleteStep)
router.delete('/steam-free/steps/:id', deleteStep)

export default router
